...
df_Bdd <- df %>% select(zone,Gr_Hotes,name_host,description,moment,last_hard_state) %>% 
  filter(str_detect(name_host,"lg") | str_detect(name_host,"ldb")) %>%  
  arrange(zone,name_host,description,moment) %>% select(-Gr_Hotes)


df_Bdd_list <- df_Bdd %>% group_by(zone,name_host,description) %>% 
  summarise(nb_alert=n()) %>% ungroup() %>% as.data.frame() %>% select(-nb_alert) 


vectheure_df <- crossing(df_Bdd_list,vectheure_df) %>% as.data.frame()

df_Bdd <- df_Bdd %>% 
  full_join(vectheure_df, by=c("zone","name_host","description","moment")) %>% arrange(zone,name_host,description, moment)



df_Bdd <- df_Bdd %>% pivot_wider(names_from=moment, values_from = last_hard_state) %>%
  as.data.frame()
df_Bdd[is.na(df_Bdd)] <- 0 

# simplification libelle...
df_Bdd <- df_Bdd %>% mutate(zone=str_sub(zone,4,str_length(zone)))



listCol <- colnames(df_Bdd) %>% as.data.frame() %>% rename(listCol=".") %>% 
  filter(!(listCol %in% c("zone","name_host","description"))) %>% mutate(listCol=str_sub(listCol,9,14)) %>% 
...
write.csv2(df_Bdd,paste0(rep_gen,"tableau_Bdd.csv"), row.names = FALSE)
write.csv2(df_Bdd,paste0(fileGen,"tableau_Bdd.csv"), row.names = FALSE)


################################################
# pour le tableau simplifie a mettre dans le mel
listCol_2 <- colnames(df_Bdd) %>% as.data.frame() %>% 
  rename(listCol=".") %>% filter(!(listCol %in% c("zone","name_host","description")))
...
for (i in 1:nrow(df_Bdd_2)){
  for (j in 4:ncol(df_Bdd_2)){
    df_Bdd_2[i,j] <- toString(unlist(df_Bdd_2[[j]][i]))
  }
}
gc()
...
for (i in 1:nrow(df_Bdd_2)){
  for (j in 4:ncol(df_Bdd_2)){
    if (str_detect(df_Bdd_2[i,j],"_2")) {
      df_Bdd_2[i,j] <- 2
    } else if (str_detect(df_Bdd_2[i,j],"_1")) {
      df_Bdd_2[i,j] <- 1
    } else if (str_detect(df_Bdd_2[i,j],"_3")) {
      df_Bdd_2[i,j] <- 3
    } else if (str_detect(df_Bdd_2[i,j],"_4")) {
      df_Bdd_2[i,j] <- 4
    } else if (str_detect(df_Bdd_2[i,j],"_5")) {
      df_Bdd_2[i,j] <- 5
    } else {
      df_Bdd_2[i,j] <- 0
    }
...
head <- paste0("<head>",
               "<meta charset='utf-8'/>",
               "<title>ENVOI INFOS SYNTHESE Base de donn&eacute;es</title>",
               "<style>",
               ...
               "thead td  {position: sticky;
                           z-index: 1; top: 0;}",
               "thead td:nth-child(2) {position: sticky;
                                       z-index: 3; left: 88px; top: 0;}",
               "thead td:nth-child(3) {position: sticky;
                                       z-index: 4; left: 210px; top: 0;}",

               "tbody tr > :first-child {position: sticky;
                                         z-index: 1; left: 0;
                                         border: 2px solid black;
                                         background-color: #fff;}",
               "tbody td:nth-child(2) {position: sticky;
                                       z-index: 1; left: 88px;
                                       border: 2px solid black;
                                       background-color: #fff;}",
               "tbody td:nth-child(3) {position: sticky;
                                       z-index: 1; left: 210px;
                                       border: 2px solid black;
                                       background-color: #fff;}",
...
titre2 <- paste0("<p style='font-size:20px'>-- Synth&egrave;se Base de donn&eacute;es --</p>")
...
# 
# if (nrow(df_Bdd)>0){
#   for (i in 1:nrow(df_Bdd)){
#   #for (i in 1:50){
#     for (j in 1:ncol(df_Bdd)){
#       print(paste0(i," et ",j," : ",df_Bdd[i,j]))
#       if ( j==1 ){
#         corps_tab <- paste0(corps_tab,"<tr>","<td class='col1'>",df_Bdd[i,j],"</td>")
#       }
#       if ( j==2 || j==3 ){
#         corps_tab <- paste0(corps_tab,"<td class='col1'>",df_Bdd[i,j],"</td>")
#       }
#       if ( j!=1 && j!=2 && j!=3 ){
#         if (df_Bdd[i,j]==1){
#           corps_tab <- paste0(corps_tab,"<td style='background-color:orange;' class='cell'>",df_Bdd[i,j],"</td>")
#         }
#         if (df_Bdd[i,j]==2){
#           corps_tab <- paste0(corps_tab,"<td style='background-color:red;' class='cell'>",df_Bdd[i,j],"</td>")
#         }
#         if (df_Bdd[i,j]==3){
#           corps_tab <- paste0(corps_tab,"<td style='background-color:grey;' class='cell'>",df_Bdd[i,j],"</td>")
#         } 
#         if (df_Bdd[i,j]==4){
#           corps_tab <- paste0(corps_tab,"<td style='background-color:#6495ed;' class='cell'>",df_Bdd[i,j],"</td>")
#         }  
#         if (df_Bdd[i,j]==5){
#           corps_tab <- paste0(corps_tab,"<td style='background-color:#87ceeb;' class='cell'>",df_Bdd[i,j],"</td>")
#         } 
#         if (!(df_Bdd[i,j] %in% c(1,2,3,4,5)) & j %in% c(  4, 16, 28, 40, 52, 64,
#                                                          76, 88,100,112,124,136,
#                                                         148,160,172,184,196,208,
#                                                         220,232,244,256,268,280)){
#           corps_tab <- paste0(corps_tab,"<td style='border:0px; 
#                                                     border-left:1px solid black;' 
#                                              class='cell'>",df_Bdd[i,j],"</td>")
#         } else if (!(df_Bdd[i,j] %in% c(1,2,3,4,5)) & j==291){
#           corps_tab <- paste0(corps_tab,"<td style='border:0px;
#                                                     border-right:1px solid black;'
#                                              class='cell'>",df_Bdd[i,j],"</td>")          
#         } else if (!(df_Bdd[i,j] %in% c(1,2,3,4,5))){
#           corps_tab <- paste0(corps_tab,"<td style='border:0px;' class='cell'>",df_Bdd[i,j],"</td>")
#         }   
#         
#       }
#       if ( j==ncol(df_Bdd) ){corps_tab <- paste0(corps_tab,"</tr>")}
#     }
#   }
# } else {
#   corps_tab <- paste0(corps_tab,"<tr><td colspan=291>absence de tableau</td></tr>")
# }
# rm(i,j)
# 
# 
...
# file <- "CorpsMel_SyntheseBdd.html"
# file.create(file, showWarnings = TRUE)
# cat(tab,file="CorpsMel_SyntheseBdd.html",append=TRUE)

# fileName <- paste0("SyntheseBdd_",date_ref_court,".html")
# file.create(fileName, showWarnings = TRUE)
# cat(tab,file=fileName,append=TRUE)

#second endroit generalise pour envoi du ficher (niveau general)
# setwd(fileGen)
# file.remove(paste0(fileGen,"SyntheseBdd.html"))
# cat(tab,file="SyntheseBdd.html",append=TRUE)
...
if (nrow(df_Bdd_2)>0){
  for (i in 1:nrow(df_Bdd_2)){
    for (j in 1:ncol(df_Bdd_2)){
      print(paste0(i," et ",j," : ",df_Bdd_2[i,j]))
      if ( j==1 ){
        corps_tab_MEL <- paste0(corps_tab_MEL,"<tr>","<td class='col1'>",df_Bdd_2[i,j],"</td>")
      }
      if ( j==2 || j==3 ){
        corps_tab_MEL <- paste0(corps_tab_MEL,"<td class='col1'>",df_Bdd_2[i,j],"</td>")
      }
      if ( j!=1 && j!=2 && j!=3 ){
        if (df_Bdd_2[i,j]==1){
          corps_tab_MEL <- paste0(corps_tab_MEL,"<td style='background-color:orange;' class='cell'>",df_Bdd_2[i,j],"</td>")
        } else if (df_Bdd_2[i,j]==2){
          corps_tab_MEL <- paste0(corps_tab_MEL,"<td style='background-color:red;' class='cell'>",df_Bdd_2[i,j],"</td>")
        } else if (df_Bdd_2[i,j]==3){
          corps_tab_MEL <- paste0(corps_tab_MEL,"<td style='background-color:#808080;' class='cell'>",df_Bdd_2[i,j],"</td>")
        } else if (df_Bdd_2[i,j]==4){
          corps_tab_MEL <- paste0(corps_tab_MEL,"<td style='background-color:#6495ed;' class='cell'>",df_Bdd_2[i,j],"</td>")
        } else if (df_Bdd_2[i,j]==5){
          corps_tab_MEL <- paste0(corps_tab_MEL,"<td style='background-color:#87ceeb;' class='cell'>",df_Bdd_2[i,j],"</td>")
        } else if (!(df_Bdd_2[i,j] %in% c(1,2,3,4,5))){
          corps_tab_MEL <- paste0(corps_tab_MEL,"<td style='border:0px;' class='cell'>",df_Bdd_2[i,j],"</td>")
        }   
      }
      if ( j==ncol(df_Bdd_2) ){corps_tab_MEL <- paste0(corps_tab_MEL,"</tr>")}
    }
  }
} else {
  corps_tab_MEL <- paste0(corps_tab_MEL,"<tr><td colspan=51>absence de tableau</td></tr>")
}
rm(i,j)
...
file <- "SyntheseBdd_MEL.html"
file.create(file, showWarnings = TRUE)
cat(tab_MEL,file="SyntheseBdd_MEL.html",append=TRUE)

#second endroit generalise pour envoi du ficher (niveau general)
setwd(fileGen)
file.remove(paste0(fileGen,"SyntheseBdd_MEL.html"))
cat(tab_MEL,file="SyntheseBdd_MEL.html",append=TRUE)
...
setwd(rep_gen)
# zip("SyntheseBdd_Docs.zip",fileName)
zip("SyntheseBdd_Docs.zip",file)
zip("SyntheseBdd_Docs.zip","tableau_Bdd.csv")
file.remove(paste0(fileGen,"SyntheseBdd_Docs.zip"))
file.copy(from = paste0(rep_gen,"SyntheseBdd_Docs.zip"),
          to = paste0(fileGen,"SyntheseBdd_Docs.zip"))


######################################################################
######################################################################

system('powershell -file "C:\\Users\\SIAR_ycg8l6\\Docs\\ProgrammesR\\CompteurAlertes_5min__Synthese\\EnvoiMail_ViaPowershell_SynthBdd.ps1"')
