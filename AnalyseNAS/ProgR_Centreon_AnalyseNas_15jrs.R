...
listing <- list.files(rep_sas) %>% as.data.frame() %>% 
  ...
  filter(str_detect(liste_fich,"ASv3_") & str_detect(liste_fich,"_15jrs") & !(str_detect(liste_fich,"jour"))) %>% 
...
listing_jour <- list.files(rep_sas) %>% as.data.frame() %>% 
  rename(liste_fich=".") %>% arrange(liste_fich) %>% 
  mutate(lg=row_number()) %>% relocate(lg) %>% 
  filter(str_detect(liste_fich,"ASv3_jour_15jrs"))

nom_json_jour <- listing_jour %>% 
  select(liste_fich) %>% as.character()
...
# et fichier jour periode
file.copy(paste0(rep_sas,nom_json_jour),paste0(folder,"/",nom_json_jour))

file.remove(paste0(rep_jour,nom_json_jour))
file.copy(paste0(rep_sas,nom_json_jour),paste0(rep_jour,"/",nom_json_jour))

file.remove(paste0(rep_sas,nom_json_jour))
...
system('powershell -file "C:\\Users\\SIAR_ycg8l6\\Docs\\ProgrammesR\\_AnalyseNas\\EnvoiMail_ViaPowershell_15jrs.ps1"')
