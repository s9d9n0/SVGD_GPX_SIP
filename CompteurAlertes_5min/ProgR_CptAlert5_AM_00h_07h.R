...
listing_jour <- list.files(rep_gen) %>% as.data.frame() %>% 
    rename(liste=".") %>%
    filter(str_detect(liste,paste0("df_datajreq_",date_ref,"_07h")) | 
           str_detect(liste,paste0("df_statusj_",date_ref,"_07h")) )
...
for (i in 1:nrow(listing_jour)){
    ...
    zip(paste0("df_jour_",date_ref,"_07h.zip"),
...
file.copy(from=paste0(rep_gen,"/df_jour_",date_ref,"_07h.zip"),
          to=paste0(rep_gen,"/df_jour_matin.zip"))
...
system('powershell -file "C:\\Users\\SIAR_ycg8l6\\Docs\\ProgrammesR\\CompteurAlertes_5min\\EnvoiMail_ViaPowershell_Tab_07h15.ps1"')

# suppression du fichier .zip envoye...
file.remove(paste0(rep_gen,"/df_jour_",date_ref,"_07h.zip"))
file.remove(paste0(rep_gen,"/df_jour_matin.zip"))
