...
listing_jour <- list.files(rep_gen) %>% as.data.frame() %>% 
    rename(liste=".") %>%
    filter(str_detect(liste,paste0("df_datajreq_",date_ref,"_17h")) | 
           str_detect(liste,paste0("df_statusj_",date_ref,"_17h")) )
...
for (i in 1:nrow(listing_jour)){
    ...
    zip(paste0("df_jour_",date_ref,"_17h.zip"),
        paste0(listing_jour$liste[i]))
    #file.remove(paste0(rep_fich,listing_dc1dc2$liste_fich[i]))
}
rm(i)

file.copy(from=paste0(rep_gen,"/df_jour_",date_ref,"_17h.zip"),
          to=paste0(rep_gen,"/df_jour_soir.zip"))
...
system('powershell -file "C:\\Users\\SIAR_ycg8l6\\Docs\\ProgrammesR\\CompteurAlertes_5min\\EnvoiMail_ViaPowershell_Tab_17h15.ps1"')

# suppression du fichier .zip envoyé...
file.remove(paste0(rep_gen,"/df_jour_",date_ref,"_17h.zip"))
file.remove(paste0(rep_gen,"/df_jour_soir.zip"))
