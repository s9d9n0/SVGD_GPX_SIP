...
listing_jour <- list.files(rep_gen) %>% as.data.frame() %>% 
    rename(liste=".") %>%
    filter(str_detect(liste,paste0("df_datajreq_",date_ref)) | 
           str_detect(liste,paste0("df_statusj_",date_ref)) )
...
for (i in 1:nrow(listing_jour)){
    ...
    zip(paste0("df_jour_",date_ref,".zip"),
...
file.copy(from=paste0(rep_gen,"/df_jour_",date_ref,".zip"),
          to=paste0(rep_gen,"/df_jour_total.zip"))
...
system('powershell -file "C:\\Users\\SIAR_ycg8l6\\Docs\\ProgrammesR\\CompteurAlertes_5min\\EnvoiMail_ViaPowershell_Tab_01h55.ps1"')

# suppression du fichier .zip envoyé...
file.remove(paste0(rep_gen,"/df_jour_",date_ref,".zip"))
file.remove(paste0(rep_gen,"/df_jour_total.zip"))
