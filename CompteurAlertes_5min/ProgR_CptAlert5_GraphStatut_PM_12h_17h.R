...
# heure_matin <- str_sub(listing$liste_fich[3],23,27)

# df_status <- read.csv2(paste0(chemin,"df_statusj_",date_ref,"_",heure_matin,".csv"))


heure_nuit <- str_sub(listing$liste_fich[1],23,27)
heure_matin <- str_sub(listing$liste_fich[2],23,27)
heure_aprem <- str_sub(listing$liste_fich[3],23,27)

df_status <- rbind(
              read.csv2(paste0(chemin,"df_statusj_",date_ref,"_",heure_nuit,".csv")),
              read.csv2(paste0(chemin,"df_statusj_",date_ref,"_",heure_matin,".csv")),
              read.csv2(paste0(chemin,"df_statusj_",date_ref,"_",heure_aprem,".csv"))
             )
...
png(file="graph_OsnyStatus.png",width = 920, height = 500)
graph_Osny
dev.off()
png(file="graph_OsnyZone.png",width = 920, height = 500)
graph_ZoneOsny
dev.off()

png(file="graph_AuzeStatus.png",width = 920, height = 500)
graph_Auzeville
dev.off()
png(file="graph_AuzeZone.png",width = 920, height = 500)
graph_ZoneAuzeville
dev.off()
...
file.copy(from=paste0(chemin_graph,"/Graph_Chronogrammes_",date_ref,".zip"),
          to=paste0(chemin_graph,"/Graph_Chronogrammes_jour_soir.zip"))
...
system('powershell -file "C:\\Users\\SIAR_ycg8l6\\Docs\\ProgrammesR\\CompteurAlertes_5min\\EnvoiMail_ViaPowershell_Graph_17h15.ps1"')

# suppression du fichier .zip envoye...
file.remove(paste0(chemin_graph,"/Graph_Chronogrammes_jour_soir.zip"))
